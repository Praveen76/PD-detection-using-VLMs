...
def construct_dataset_df(config):
    ...
    landmark_features_path = "/localdisk1/PARK/park_vlm/SmileFeatures/LandmarkFeatures"
    df_landmark = pd.read_csv(os.path.join(landmark_features_path, "mediapipe_features_all_task.csv"))
    ...
    openface_features_path = "/localdisk1/PARK/park_vlm/SmileFeatures/OpenfaceFeatures"
    df_openface = pd.read_csv(os.path.join(openface_features_path, "openface_features_all_task.csv"))
    ...
    df_openface = df_openface.drop(columns=["filepath", "date", "id", "task_name"])
    df_landmark = df_landmark.drop(columns=["filepath", "date", "id", "task_name"])

    df_smile_features = df_openface.set_index("filename").join(df_landmark.set_index("filename"), on="filename", how="inner", lsuffix='_openface', rsuffix='_landmark')
    df_smile_features = df_smile_features.reset_index()

    print(f"{len(df_smile_features.columns)} features, {len(df_smile_features)} data")
    
    # Identify null features
    # size_of_all_data = len(df_smile_features)
    # for feature in df_smile_features.columns:
    #     size_of_data_with_feature = len(df_smile_features.dropna(subset=[feature]))
    ...
    df_smile_features = df_smile_features.dropna()

    print(f"After null drop: {len(df_smile_features.columns)} features, {len(df_smile_features)} data")

    '''
    Drop metadata columns to focus on features
    '''
    df_features = df_smile_features.drop(columns=['filename'])
    ...
    if config["drop_correlated_features"]=="yes":
        ...
        # print("Dropped smile features")
        # for x in list(drops):
        #     print(x)
        # print("##"*10)
        
        # Drop features from both the main and the feature dataframe
        df_smile_features.drop(drops, axis=1, inplace=True)
        df_features.drop(drops, axis=1, inplace=True)

    print(f"Size of smile features: {len(df_smile_features)}")
    print(f"Number of smile features: {len(df_smile_features.columns)-1}")
    feature_names = df_smile_features.columns[1:]

    # combined embedding and label columns into a single dataframe
    for fold in ["train", "dev", "test"]:
        dataset[fold] = dataset[fold].set_index("file_name").join(df_smile_features.set_index("filename"), on="file_name", how="inner")
...
@click.option("--dataset_name", default="smile")
def main(**cfg):
    ...
    # for x in dataset_df["train"].columns:
    #     print(x)
